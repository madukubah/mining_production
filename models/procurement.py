...
class ProcurementRule(models.Model):
    ...
    def _get_action(self):
        return [('mining_production', _('Mining Production'))] + super(ProcurementRule, self)._get_action()
...
class ProcurementOrder(models.Model):
    ...
    def propagate_cancels(self):
        cancel_mining_production_orders = self.filtered(lambda procurement: procurement.rule_id.action == 'mining_production' and procurement.production_order_id).mapped('production_order_id')
        if cancel_mining_production_orders:
            cancel_mining_production_orders.action_cancel()
        return super(ProcurementOrder, self).propagate_cancels()

    # @api.multi
    # def _run(self):
    #     self.ensure_one()
    #     if self.rule_id.action == 'mining_production':
    ...
    @api.multi
    def _check(self):
        return self.production_order_id.state == 'done' or super(ProcurementOrder, self)._check()
